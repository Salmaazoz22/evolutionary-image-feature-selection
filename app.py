...
with tab1:
    ...
    for alg_name, res, color in show:
        ...
        if baseline_acc is not None:
            if acc >= baseline_acc:
                interp = "✅ **Great result!** The algorithm matched or exceeded the baseline accuracy using fewer features."
            elif acc >= baseline_acc - 0.02:
                pct_drop = (baseline_acc - acc) * 100
                interp = (
                    f"✅ **Good trade-off:** only {pct_drop:.1f}% accuracy drop "
                    f"but {reduction:.1f}% fewer features used."
                )
            else:
                interp = (
                    "⚠️ **Accuracy dropped significantly.** "
                    "Try increasing Generations/Iterations or Population Size."
                )
            st.info(
                interp + "\n\n"
                "**PSO tip:** try more iterations.  "
                "**GA tip:** try Tournament selection + One-Point crossover."
            )
